...
HOST=$HOSTNAME
#OS=$(uname -a)
OS=$(lsb_release -s -i -c -r | sed -z 's/\n/ /g')
UPTIME=$(uptime)
ARCHITECTURE=$(uname -m)

#Cache 
L1icache=$(lscpu | egrep '^L1i cache:' | awk '{print $3}')
L1dcache=$(lscpu | egrep '^L1d cache:' | awk '{print $3}')
L2cache=$(lscpu | egrep '^L2 cache:' | awk '{print $3}')
L3cache=$(lscpu | egrep '^L3 cache:' | awk '{print $3}')
...
JSON="
{
  ...
  \"meomory_used\": $memUsed,
  \"memory_cache\": $memCached,
  \"memory_swap\": $swapUsed,
